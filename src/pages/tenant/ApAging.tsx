...
import { ApAgingReport, AgingBucket } from '../../types/aging';
...
export function ApAging() {
  ...
  useEffect(() => {
    api
      .get<ApAgingReport>('/tenant/ap-aging')
      .then(setData)
      .catch((err) => toast.error(err instanceof ApiError ? err.message : 'Failed to load AP aging report'))
        ...
        <>
            ...
            <div className="space-y-3 p-5">
              {data.byBucket.map((b) => (
                <div key={b.bucket} className="flex items-center gap-3">
                  ...
                  <p className="w-16 shrink-0 text-right text-xs text-text-gray dark:text-slate-400">{b.count} PO{b.count === 1 ? '' : 's'}</p>
                </div>
              ))}
            </div>
          </Card>
          ...
          <Card>
            {data.suppliers.length === 0 ? (
              <div className="p-5"><EmptyState icon={WalletIcon} title="Nothing owed" description="Every supplier payment is fully settled." /></div>
            ) : (
                  ...
                  <tbody>
                    {data.suppliers.map((s) => (
                      <tr key={s.id} className="border-b border-border-soft last:border-0 dark:border-slate-800">
                        ...
                        <td className="px-5 py-3"><Badge tone={BUCKET_TONE[s.oldestBucket]}>{s.oldestBucket}</Badge></td>
                        <td className="px-5 py-3 text-right font-bold text-navy dark:text-slate-100">{formatCurrency(s.outstanding)}</td>
                      </tr>
                    ))}
                  </tbody>
                </table>
