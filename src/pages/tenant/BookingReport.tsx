...
import { CalendarIcon, ClipboardCheckIcon, XCircleIcon, GlobeIcon } from 'lucide-react';
import { PageHeader } from '../../components/ui/PageHeader';
import { Card, CardHeader } from '../../components/ui/Card';
import { StatCard } from '../../components/ui/StatCard';
import { Badge } from '../../components/ui/Badge';
import { Skeleton } from '../../components/ui/Skeleton';
import { BookingReport as BookingReportData } from '../../types/bookingReport';
...
export function BookingReport() {
  ...
  useEffect(() => {
    ...
    api
      .get<BookingReportData>(`/tenant/booking-report?${query}`)
      .then(setData)
      .catch((err) => toast.error(err instanceof ApiError ? err.message : 'Failed to load booking report'))
      .finally(() => setLoading(false));
  }, [range, customFrom, customTo]);

  return (
    <div>
      <PageHeader title="Booking Report" description="Booking volume, conversion, and cancellations." />
      ...
      {loading || !data ? (
        ...
        <>
          <div className="mb-6 grid grid-cols-2 gap-4 sm:grid-cols-4">
            <StatCard label="Total bookings" value={String(data.total)} icon={CalendarIcon} />
            <StatCard label="Converted to job" value={`${data.conversionRate}%`} icon={ClipboardCheckIcon} />
            <StatCard label="Cancelled" value={`${data.cancellationRate}%`} icon={XCircleIcon} />
            <StatCard label="Booked online" value={`${data.onlineSharePct}%`} icon={GlobeIcon} />
          </div>

          <Card className="mb-6">
            <CardHeader title="Daily booking volume" />
            <div className="h-64 p-5 pt-0">
              <ResponsiveContainer width="100%" height="100%">
                <BarChart data={data.dailyVolume}>
                  ...
                  <Tooltip />
                  <Bar dataKey="count" name="Bookings" fill="#2A8BD4" radius={[4, 4, 0, 0]} />
                </BarChart>
              </ResponsiveContainer>
            </div>
          </Card>
          ...
          <Card>
            ...
            <div className="flex flex-wrap gap-2 p-5">
              {Object.entries(data.byStatus).map(([status, count]) => (
                <Badge key={status} tone={status === 'Cancelled' ? 'red' : status === 'Completed' ? 'green' : 'blue'}>
                  {status}: {count}
                </Badge>
              ))}
            </div>
