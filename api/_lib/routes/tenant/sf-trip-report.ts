...
// One row per visit with a computed trip distance (SF-Phase 10) — omits
// visits with no prior geo-tagged stop to compare against (null distance),
// same as the SF Dashboard's trip rollup.
export default async function handler(req: VercelRequest, res: VercelResponse) {
  ...
  const rows = visits
    ...
    .filter((r) => r.tripDistanceKm != null);

  const totalDistanceKm = Math.round(rows.reduce((sum, r) => sum + (r.tripDistanceKm ?? 0), 0) * 100) / 100;
  const totalFuelCost = Math.round(rows.reduce((sum, r) => sum + (r.estimatedFuelCost ?? 0), 0) * 100) / 100;
  const visitsWithFuelEstimate = rows.filter((r) => r.estimatedFuelCost != null).length;

  return res.status(200).json({
    range: { from, to },
    summary: {
      visitsWithDistance: rows.length,
      totalDistanceKm,
      totalFuelCost,
      visitsWithFuelEstimate,
      avgDistanceKm: rows.length > 0 ? Math.round((totalDistanceKm / rows.length) * 100) / 100 : 0,
    },
    rows,
  });
}
