...
async function handleList(req: VercelRequest, res: VercelResponse) {
  ...
  const statsBySupplier = new Map<string, { openOrders: number; lastOrder: Date | null; received: number; onTimeCount: number; onTimeEligible: number }>();
  for (const order of orders) {
    const key = order.supplierId.toString();
    const stat = statsBySupplier.get(key) ?? { openOrders: 0, lastOrder: null, received: 0, onTimeCount: 0, onTimeEligible: 0 };
