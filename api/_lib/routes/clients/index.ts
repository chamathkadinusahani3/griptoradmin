...
async function handleCreate(req: VercelRequest, res: VercelResponse) {
  ...
  try {
    ...
    await session.withTransaction(async () => {
        ...
        [
          {
            ...
            staff: staff ?? 1,
            mrr: await computeMrr(plan, status),
            slug,
          },
        ],
        { session }
      ...
      await User.create(
        [{ name: contact, email: normalizedEmail, passwordHash, role: 'tenant', clientId: client._id, tenantRole: 'Owner', roleId: ownerRole._id }],
        { session }
      );
